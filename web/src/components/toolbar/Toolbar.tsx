...
export function Toolbar({ onRender }: ToolbarProps) {
  ...
  const { renderStatus, previewMode, autoRender, codePanelOpen, setPreviewMode, setAutoRender, toggleCodePanel } = useEditorStore()

  return (
    <header className="h-10 bg-gray-950 border-b border-white/10 flex items-center gap-4 px-4 shrink-0">
      {/* Brand */}
      <div className="flex items-center gap-2 mr-2">
        ...
        <span className="text-[10px] text-gray-500 hidden sm:block">OpenSCAD Visual Designer</span>
      </div>
