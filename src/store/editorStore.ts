...
interface EditorState {
  ...
  propagateForLoopContext: (
    forLoopNodeId: string,
    patch: Partial<{
      varName: string;
      start: string;
      end: string;
  ...
  /** Create a loop body tab silently (without navigating to it). Returns tab id. */
  createLoopBodyTab: (
    label: string,
    seedNodes: Node[],
  ...
  updateGlobalParameter: (
    id: string,
    patch: Partial<Omit<GlobalParameter, "id">>,
  ) => void;
...
export const useEditorStore = create<EditorState>()(
  immer((set, get) => {
    ...
    return {
            ...
            if (tab.tabType === "module" || tab.tabType === "loop") {
                ...
                for (const t of state.tabs) {
                  for (const node of t.nodes) {
                    if (
                      ...
                      (node.data as Record<string, unknown>).moduleName ===
                        oldModuleName
                    ) {
                      (node.data as Record<string, unknown>).moduleName =
                        newSanitized;
              ...
              if (oldSketchName && oldSketchName !== newSanitized) {
                for (const t of state.tabs) {
                  for (const node of t.nodes) {
                    if (
                      node.type === "sketch_profile" &&
                      (node.data as Record<string, unknown>).sketchName ===
                        oldSketchName
                    ) {
                      (node.data as Record<string, unknown>).sketchName =
                        newSanitized;
      ...
      onNodesChange: (changes) =>
        set((state) => {
          ...
          const removedBodyTabIds = changes
            ...
            .map((c) =>
              state.nodes.find((n) => n.id === (c as { id: string }).id),
            )
            .filter((n) => n?.type === "for_loop")
            .map(
              (n) =>
                (n?.data as Record<string, unknown>)?.bodyTabId as
                  | string
                  | undefined,
            )
      ...
      propagateForLoopVarName: (forLoopNodeId, newVarName) =>
        set((state) => {
          ...
          if (forLoopNode) {
            ...
            const bodyTabId = (forLoopNode.data as Record<string, unknown>)
              .bodyTabId as string | undefined;
            if (bodyTabId) {
              const bodyTab = state.tabs.find((t) => t.id === bodyTabId);
              if (bodyTab) {
                for (const n of bodyTab.nodes) {
                  if (n.type === "loop_var" || n.type === "loop_context") {
      ...
      propagateForLoopContext: (forLoopNodeId, patch) =>
        set((state) => {
          const forLoopNode = state.nodes.find((n) => n.id === forLoopNodeId);
          if (!forLoopNode) return;
          // Update the for_loop node's own fields
          Object.assign(forLoopNode.data as Record<string, unknown>, patch);
          const bodyTabId = (forLoopNode.data as Record<string, unknown>)
            .bodyTabId as string | undefined;
          if (!bodyTabId) return;
          const bodyTab = state.tabs.find((t) => t.id === bodyTabId);
          if (bodyTab) {
            for (const n of bodyTab.nodes) {
              if (n.type === "loop_context") {
                Object.assign(n.data as Record<string, unknown>, patch);
              }
            }
          }
          // Also sync if the body tab is currently the active tab
          if (state.activeTabId === bodyTabId) {
            for (const n of state.nodes) {
              if (n.type === "loop_context") {
                Object.assign(n.data as Record<string, unknown>, patch);
      ...
      groupSelectedNodes: () =>
        set((state) => {
          const selected = state.nodes.filter(
            (n) => n.selected && n.type !== "group_node",
          );
      ...
      setShowParametersPanel: (v) =>
        set((state) => {
          state.showParametersPanel = v;
        }),
      setPreviewMode: (m) =>
        set((state) => {
          state.previewMode = m;
      ...
      setSketchPreviewSvg: (svg) =>
        set((state) => {
          state.sketchPreviewSvg = svg;
        }),
      sketchGeneratedCode: "",
      setSketchGeneratedCode: (code) =>
        set((state) => {
          state.sketchGeneratedCode = code;
        }),
      ...
      importProject: (json) =>
        set((state) => {
          try {
            ...
            state.globalParameters = Array.isArray(data.globalParameters)
              ? data.globalParameters
              : [];
            state.projectName =
              typeof data.projectName === "string" ? data.projectName : "";
            state.importedFiles =
              data.importedFiles && typeof data.importedFiles === "object"
                ? data.importedFiles
                : {};
