...
export function GridCopiesNode({ id, data, selected }: NodeProps) {
  ...
  return (
    <BaseNode
      ...
      inputHandles={[
        { id: "in-0", label: "child" },
        { id: "in-1", label: "spacing_x" },
        { id: "in-2", label: "spacing_y" },
        { id: "in-3", label: "n_x" },
        { id: "in-4", label: "n_y" },
      ]}
    >
      <ExpressionInput
        ...
        value={d.spacing_x}
        step={1}
        onChange={(v) => update(id, { spacing_x: v })}
      />
      <ExpressionInput
        label="spacing_y"
        value={d.spacing_y}
        step={1}
        onChange={(v) => update(id, { spacing_y: v })}
      />
      <ExpressionInput
        label="n_x"
        value={d.n_x}
        step={1}
        onChange={(v) => update(id, { n_x: v })}
      />
      <ExpressionInput
        label="n_y"
        value={d.n_y}
        step={1}
        onChange={(v) => update(id, { n_y: v })}
      />
      <CheckboxInput
        label="stagger"
        value={d.stagger}
        onChange={(v) => update(id, { stagger: v })}
      />
    </BaseNode>
  );
}
